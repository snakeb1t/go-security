...
var _ = Describe("Pkcs11SSL", func() {
	...
	var genericBeforeEach = func() {
		...
		conf = &Config{
			PKCS11Slot:       374292918,
			PKCS11DriverFile: lib,
		}

		prov, err = New(WithChoriaConfig(c), WithPKCSConfigOptions(conf), WithLog(l.WithFields(logrus.Fields{})), WithPin(pin))
		Expect(err).ToNot(HaveOccurred())
	}
	...
	Describe("WithChoriaConfig", func() {
		It("Should copy all the relevant settings", func() {
			...
			prov, err := New(WithChoriaConfig(c), WithPKCSConfigOptions(conf), WithLog(l.WithFields(logrus.Fields{})), WithPin("1234"))
			Expect(err).ToNot(HaveOccurred())
	...
	Describe("VerifyCertificate", func() {
		...
		It("Should work with server side ca intermediate chains", func() {
			prov, err := New(WithChoriaConfig(inter), WithPKCSConfigOptions(conf), WithLog(l.WithFields(logrus.Fields{})), WithPin("1234"))
			Expect(err).ToNot(HaveOccurred())
		...
		It("Should work with email addresses", func() {
			prov, err := New(WithChoriaConfig(inter), WithPKCSConfigOptions(conf), WithLog(l.WithFields(logrus.Fields{})), WithPin(pin))
			Expect(err).ToNot(HaveOccurred())
